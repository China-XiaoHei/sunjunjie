...
A2 目录只保存部署说明，不复制 A1 源码。实际运行的 `server.js`、`controller.py`、`path.json` 和 WSL 脚本均位于同级目录 `题目A1`；本说明中的所有启动命令都先进入 `题目A1`。
...
外部控制器只使用 Python 标准库，不需要 `pip install`，也不需要 `requirements.txt`。
...
上述命令运行的是 `题目A1/controller.py`，不是 `题目A2` 下的独立程序。也可以在仓库根目录直接执行：

```powershell
cd F:\实践考题
python .\题目A1\controller.py --host 127.0.0.1 --port 8080 --speed 0.35
```
